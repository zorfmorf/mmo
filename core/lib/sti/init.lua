...
function Map:setTiles(index, tileset, gid)
	...
	for y = 1, h do
		for x = 1, w do
            ...
            -- FIXME BEGIN
            if not self.tileanims then self.tileanims = {} end
            if tile.animation then table.insert(self.tileanims, gid) end
            -- FIXME END
...
function Map:update(dt)
    -- FIXME BEGIN
	for _, t in pairs(self.tileanims) do
        
        local tile = self.tiles[t]
		local update = false
